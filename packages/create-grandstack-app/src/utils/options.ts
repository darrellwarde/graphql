...
const templateOpts = ["React", "React-TS", "API-Only"];
...
const getRmTemplates = (chosenTemplate) => {
    return Object.values(templateFileNameHashMap).filter(
        (name) => name !== templateFileNameHashMap[chosenTemplate] && name !== "api-only"
    );
};
...
export const promptForMissingOptions = async (options) => {
    ...
    if (skipPrompts) {
        return {
            ...
            rmTemplates: getRmTemplates(template),
            templateName: template || defaultTemplate,
            templateFileName: getTemplateFileName(template),
